...
def main():
    ...
    parser.add_argument("--data_dir", type=str, default="/hpc2hdd/home/ywang183/AttnSleep/data/EDF153/",
                        help="File path to the PSG and annotation files.")
    parser.add_argument("--output_dir", type=str, default="data_edf153_dual_fpzcz_eogh",
                        help="Directory where to save numpy files outputs.")
    parser.add_argument("--select_ch", type=str, default="EEG Fpz-Cz",
                        help="The selected EEG channel")
    parser.add_argument("--select_eog", type=str, default="EOG horizontal",
                        help="The selected EOG channel")
    ...
    for i in range(len(psg_fnames)):
        ...
        # 提取EOG数据
        raw_eog_df = raw.to_data_frame()[select_eog]
        raw_eog_df = raw_eog_df.to_frame()
        raw_eog_df.set_index(np.arange(len(raw_eog_df)))
        ...
        for a in ann[0]:
            ...
            if label != UNKNOWN:
                if duration_sec % EPOCH_SEC_SIZE != 0:
                    raise Exception("Duration not divisible by epoch size")
        ...
        if len(raw_ch) % (EPOCH_SEC_SIZE * sampling_rate) != 0:
            raise Exception("Data length not divisible by epoch size")
